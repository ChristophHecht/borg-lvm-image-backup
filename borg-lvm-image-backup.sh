#!/bin/bash
# BorgBackup LVM Image Backup Script V1.0
...
##RUNNING ZEROFREE
echo "$(date +"%Y-%m-%d-%H-%M-%S"):----RUNNING ZEROFREE----" | tee -a $LOGFILE
zerofree $SNAPSHOTPATH | tee -a $LOGFILE
if [ $PIPESTATUS -ne 0 ]
then
...
echo "$(date +"%Y-%m-%d-%H-%M-%S"):----FINISHED RUNNING ZEROFREE----" | tee -a $LOGFILE

##DO BACKUP
echo "$(date +"%Y-%m-%d-%H-%M-%S"):----RUNNING BACKUP JOB----" | tee -a $LOGFILE
$BORGLOCATION create --read-special --info --compression $COMPRESSION --stats $REPOSITORY::$JOBNAME-$TIMESTAMP $SNAPSHOTPATH 2>&1 >/dev/null | tee -a $LOGFILE
